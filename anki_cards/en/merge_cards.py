...
def merge_csv_files():
    ...
    'all_anki_cards_en.csv' file.
    """
    output_filename = 'all_anki_cards_en.csv'
    # Get a list of all CSV files to merge in the current directory
    csv_files = sorted([f for f in os.listdir('.') if f.endswith('_cards.csv')])
    
    if not csv_files:
        print("No '*_cards.csv' files found in the current directory to merge.")
        return
